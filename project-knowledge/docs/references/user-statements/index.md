...
- [外部レイヤーパスの許可](2026-09-16-external-layer-paths.md): プロジェクト外のレイヤーを許可する範囲、拒否を維持する入力、外部書込みとignoreの境界を確認するときに読む。
- [複数レイヤーの初期化と分割フロー](2026-09-15-multi-layer-workflows.md): 複数レイヤー初期設定と既存ナレッジの分割処理を実装する依頼を確認するときに読む。
- [初期化時の既定レイヤー名](2026-09-15-default-layer-name.md): 初期化で生成する`id: default`レイヤーの`name`を`プロジェクトナレッジ`に固定する方針を確認するときに読む。
