...
- `knowledge-policy.md`: frontmatterに運用設定、本文に収集・品質Policyを持つ。
...
`knowledge.human_readable`はboolean、`learning.mode`は`manual`、`opportunistic`、`aggressive`のいずれかとする。設定変更は既知キーだけを更新し、本文、コメント、未知キーを保持する。不正なfrontmatterは自動修復しない。
