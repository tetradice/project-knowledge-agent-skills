# プロジェクトナレッジ Skill

- [Versioning contracts](versioning-contracts.md): Project Knowledge、Skill、Knowledge format、OKF、state schemaの独立した版契約と、未定義のリポジトリリリース境界を確認するときに読む。

このカテゴリのKnowledgeを選ぶための索引です。

- [プロジェクトナレッジ Skill概要](overview.md): メインSkillと専用help・inspect、verify/fix、audit/refactor、Policy、provenance、learningを確認するときに読む。
- [Project Knowledgeのルート設定](root-configuration.md): `project-knowledge.yaml`による対象判定、レイヤー解決、読み取り専用の既定値、初期生成設定を確認するときに読む。
- [複数レイヤーの初期化と分割](multi-layer-workflows.md): 固定したレイヤー定義と配置表、設定の最後の公開、分割時の移管規則、ロックと復旧を確認するときに読む。
- [未登録の旧来ナレッジを移行する](legacy-knowledge-migration.md): `project-knowledge.yaml`がない旧来ナレッジを形式1.0のBundleへ準備し、確認後に登録する手順を確認するときに読む。
- [init / updateのファイル変更報告](file-change-reporting.md): 追加・更新ファイルの共通分類、利用者向け件数、Support/Internalの非表示境界を確認するときに読む。
- [Project Knowledge形式1.0](format-1.0.md): manifest、Policy frontmatter、OKF bundle、publish既定値を確認するときに読む。
- [仕様相談用テキストの再生成](specification-overview-regeneration.md): 外部相談用の仕様概要を同じ方針で再作成するときに読む。
- [再構築可能なProject Knowledge state](rebuildable-state.md): stateの最小schema、Git baseline、非Git snapshot、復旧、validator境界を確認するときに読む。
- [QuickシナリオのモデルBenchmark](model-benchmark.md): Luna/Terra/SolによるQuick Knowledge Base構築の比較、固定blind Judge、token usageの扱いを確認するときに読む。
- [Project Knowledge Utility Benchmark](utility-benchmark.md): 任意Git repositoryのNo-Knowledge / With-Knowledge Task比較、worktree隔離、blind Judge、実測usageを確認するときに読む。
- [Largeライフサイクルシナリオ](large-lifecycle-scenario.md): Quickと同じ品質観点を人工Fixture、複数update、step別token・規模推移で評価するときに読む。
- [通常初期化の品質ゲート](normal-initialization-quality-gate.md): 通常初期化の完了条件、Quickの決定論的な最低契約、直近のQuick実行結果を確認するときに読む。
- [Scenario Testの配置](scenario-test-placement.md): 一般ユーザー向けSkillから分離した開発者用テストの配置と実行境界を確認するときに読む。
