...
def prepare(repository: Path, task_file: Path, baseline: str = "HEAD", output_root: Path | None = None, checks_file: Path | None = None) -> Path:
    ...
    raw_config = run_git(repository, "show", f"{commit}:{CONFIG_NAME}", check=False)
    if raw_config.returncode != 0:
        raise BenchmarkError(f"baseline is not registered: {CONFIG_NAME} missing")
    try:
