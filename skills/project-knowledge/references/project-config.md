...
| `layers[].path` | 必須。設定所在地から解決する相対パスまたは絶対パス。管理ディレクトリを指す |
...
相対パスは設定所在地から解決し、`..`による上位ディレクトリ、絶対パス、UNC、シンボリックリンク／junction経由の実体パスを登録できる。
URL、環境変数、`~`、globは受け付けない。プロジェクトルート自身はレイヤーにできない。
